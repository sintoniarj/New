...
export async function GET(request: NextRequest) {
  ...
  const baseUrl = requestOrigin || process.env.NEXT_PUBLIC_URL || 'https://lapso.pw'
  ...
  // Process verification server-side using verifyOtp
  if (token && type) {
    try {
      ...
      if (data.user) {
        ...
        if (pendingReferralCode) {
          try {
            ...
            console.log('✅ Added referral code to user:', pendingReferralCode)
            shouldClearReferralCookie = true
          } catch (error) {
            console.error('Failed to add referral code to user:', error)
          }
        }
      ...
      return NextResponse.redirect(`${baseUrl}/auth?error=verification_failed`)
    }
  }
